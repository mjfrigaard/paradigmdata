...
I've previously built 1) a 'minimal' shiny app ([`VanillaApp`](https://mjfrigaard.github.io/posts/vanilla-shiny/)), and 2) a shiny app built as an R package ([`myPkgApp`](https://mjfrigaard.github.io/posts/my-pkg-app/)). The GitHub repo with all shiny app setups is [here](https://github.com/mjfrigaard/shiny-app-setups).
...
#| echo: false
#| results: asis
co_box(color = "o", "`golem` = pakcage", contents = "
...
The `golem-config.yml` file is covered below, but the `inst/app/` folder works just like the `inst/extdata` folder (it is loaded when the package is installed and makes these files available to the application).
